...
include("db.php");
if ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'warden') { die("Unauthorized access.");
}
...
while ($row = mysqli_fetch_assoc($result)) { fputcsv($output, [$row['full_name'], $row['email'], $row['phone']]);
}
fclose($output);
exit;