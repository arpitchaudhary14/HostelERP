...
if(isset($_GET['action']) && isset($_GET['id'])){ $id = intval($_GET['id']); $action = $_GET['action']; if($action === "approve"){ $stmt = mysqli_prepare($conn, "UPDATE leave_requests SET status='Approved' WHERE id=?"); mysqli_stmt_bind_param($stmt, "i", $id); mysqli_stmt_execute($stmt); } if($action === "reject"){ $stmt = mysqli_prepare($conn, "UPDATE leave_requests SET status='Rejected' WHERE id=?"); mysqli_stmt_bind_param($stmt, "i", $id); mysqli_stmt_execute($stmt); }
}
$query = "
...
<h4 class="mb-3" style="font-weight:700; ">Leave Requests</h4>
