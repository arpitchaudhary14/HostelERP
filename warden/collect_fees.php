...
if(isset($_POST['collect'])){ validate_csrf(); $fee_id = intval($_POST['fee_id']); $method = mysqli_real_escape_string($conn, $_POST['method'] ?? 'Cash'); $stmt = mysqli_prepare($conn, "UPDATE fees SET status='Paid', paid_on=CURDATE() WHERE id=?"); mysqli_stmt_bind_param($stmt, "i", $fee_id); mysqli_stmt_execute($stmt); $success = "Fee marked as Paid.";
}
...
<thead class="">
<tr> <th>Student</th> <th>Amount</th> <th>Due Date</th> <th>Status</th> <th>Action</th>
</tr>
</thead>
<tbody>
...
<select name="method" class="form-select form-select-sm" style="width:auto;"> <option>Cash</option> <option>UPI</option> <option>Bank</option>
</select>
