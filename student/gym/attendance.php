...
$this_month = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM gym_attendance WHERE user_id = $user_id AND MONTH(date) = MONTH(CURDATE()) AND YEAR(date) = YEAR(CURDATE())"))['total'] ?? 0;
?>
    ...
    <div class="row g-4 mb-4">
        <div class="col-md-6 reveal">
            <div class="glass-card-light h-100 d-flex flex-column justify-content-center">
                <div class="text-center py-3">
                    <h5 class="fw-bold mb-3">Gym Check-in</h5>
                    <form method="POST">
                    ...
                    <p class="text-muted small mt-3">
                        <?= $current_session ? 'You are currently active in the gym.' : 'Ready for your workout? Click above to check in.' ?>
                    </p>
                </div>
            </div>
        </div>
        <div class="col-md-3 reveal">
            <div class="stat-card stat-primary">
                ...
                <small class="text-muted">All-time gym visits</small>
            </div>
        </div>
        <div class="col-md-3 reveal">
            <div class="stat-card stat-info">
                <h5>This Month</h5>
                <h2 style="color:var(--accent-info);"><?= $this_month ?></h2>
                <small class="text-muted">Sessions in <?= date('F') ?></small>
            </div>
        </div>
    </div>
    <div class="glass-card-light reveal">
        <h4 class="mb-4 fw-bold">Attendance History</h4>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $row['check_out_time'] ? date('h:i A', strtotime($row['check_out_time'])) : '<span class="text-success small">In Gym</span>' ?></td>
                        <td>
                            <?php 
                    ...
                    <?php if(mysqli_num_rows($logs) == 0){ ?>
                        <tr><td colspan="5" class="text-center text-muted py-4">No attendance records found. Start your first session today!</td></tr>
                    <?php } ?>
                </tbody>
            </table>
        </div>
