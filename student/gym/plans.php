...
<div class="container mt-4 page-fade-in">
    <div class="glass-card-light mb-4 reveal text-center" style="padding:var(--space-2xl);">
        <img src="/WebTechProject/assets/images/MatrixFit_Logo.jpeg" height="60" class="mb-3" style="border-radius: 12px;">
        <h2 style="font-weight:800; color:#1a1a2e;">Level Up Your Fitness</h2>
        <p style="color:#666; max-width: 600px; margin: 0 auto;">Choose a membership plan that fits your goals and join the MatrixFit community today.</p>
    </div>
    <?php if($message){ ?>
    ...
    <?php if($active_sub){ ?>
        <div class="alert alert-info text-center reveal">
            You already have an active <strong><?= htmlspecialchars($active_sub['plan_name']) ?></strong> membership.
            <br><small>Expires on: <?= date('d M, Y', strtotime($active_sub['end_date'])) ?></small>
        </div>
    <?php } ?>
    <div class="row g-4 justify-content-center">
        ...
        <div class="col-md-4 reveal">
            <div class="glass-card-light h-100 d-flex flex-column text-center">
                ...
                <div class="mt-4">
                    <?php if($active_sub && $active_sub['plan_id'] == $plan['id']){ ?>
                        <button class="btn btn-success w-100 rounded-pill py-2 fw-bold" disabled>Active Plan</button>
                    <?php } else { ?>
                        <a href="../payment_gateway.php?type=gym_sub&id=<?= $plan['id'] ?>&amount=<?= $plan['price'] ?>&item=<?= urlencode($plan['name']) ?> Membership" class="btn btn-gradient w-100 rounded-pill py-2 fw-bold">Pay & Join</a>
                    <?php } ?>
                </div>
            </div>
        </div>
