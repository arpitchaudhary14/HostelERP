...
include("../db.php");
if($_SESSION['role'] != 'admin' && $_SESSION['role'] != 'warden'){ header("Location: ../dashboard.php"); exit();
}
...
<div class="glass-card-light mb-2"> <div class="d-flex justify-content-between"> <h6 class="mb-0"><?= htmlspecialchars($n['title']) ?></h6> <div> <small class="text-muted me-2"><?= htmlspecialchars($n['created_at']) ?></small> <a href="../delete_notice.php?id=<?= $n['id'] ?>" class="btn btn-sm btn-outline-danger py-0" onclick="return confirm('Are you sure you want to delete this notice?')">Delete</a> </div> </div> <p class="mb-0 mt-2"><?= htmlspecialchars($n['message']) ?></p>
</div>
<?php endforeach; ?>
</div>
