...
$msg = ""; $error = "";
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['items'])) { $day = mysqli_real_escape_string($conn, $_POST['day_of_week']); $meal = mysqli_real_escape_string($conn, $_POST['meal_type']); $items = mysqli_real_escape_string($conn, $_POST['items']); $stmt = mysqli_prepare($conn, "INSERT INTO mess_menu (day_of_week, meal_type, items, updated_by) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE items=VALUES(items), updated_by=VALUES(updated_by)"); mysqli_stmt_bind_param($stmt, "sssi", $day, $meal, $items, $user_id); if(mysqli_stmt_execute($stmt)){ $msg = "Menu updated successfully!"; } else { $error = "Failed to update menu."; }
}
$menu_result = mysqli_query($conn, "SELECT * FROM mess_menu");
$menu = [];
while($row = mysqli_fetch_assoc($menu_result)){ $menu[$row['day_of_week']][$row['meal_type']] = $row['items'];
}
