...
if(isset($_GET['action']) && isset($_GET['id'])){
    ...
    $action = $_GET['action'];
    if($action === "approve"){
        $stmt = mysqli_prepare($conn, "UPDATE warden_leave_requests SET status='Approved' WHERE id=?");
        mysqli_stmt_bind_param($stmt, "i", $id);
        mysqli_stmt_execute($stmt);
    }
    if($action === "reject"){
        $stmt = mysqli_prepare($conn, "UPDATE warden_leave_requests SET status='Rejected' WHERE id=?");
        mysqli_stmt_bind_param($stmt, "i", $id);
        mysqli_stmt_execute($stmt);
    }
}
